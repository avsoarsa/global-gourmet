-- Comprehensive SQL query to set up the coupon system
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'tax_amount') THEN
        ALTER TABLE public.orders ADD COLUMN tax_amount DECIMAL(10, 2) NOT NULL DEFAULT 0;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'orders' AND column_name = 'shipping_amount') THEN
        ALTER TABLE public.orders ADD COLUMN shipping_amount DECIMAL(10, 2) NOT NULL DEFAULT 0;
    END IF;
...
DROP POLICY IF EXISTS admin_coupons_policy ON public.coupons;
DROP POLICY IF EXISTS user_coupons_policy ON public.coupons;
...
-- Create or replace function to validate coupon
...
-- Create or replace function to apply coupon to order
